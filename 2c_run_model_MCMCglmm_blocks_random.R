...
start.time <- Sys.time()
model <- MCMCglmm(
  fixed = cbind(trunk, tail) ~ trait - 1,  
...
end.time - start.time # about 8 minutes
...
V_cols <- grep("block|sire|dam|interaction|units", colnames(model$VCV), value = TRUE)
...
# Create PDF
pdf("Model_outputs/Model_I_MCMCglmm_blocks_random_density_trace.pdf", width = 7, height = 8)
...
# Matrices
VBmat <- extract_matrix(model$VCV, format_labels("block"))
VAmat <- extract_matrix(model$VCV, format_labels("block:sire"), scale = 4)
VMmat <- extract_matrix(model$VCV, format_labels("block:dam"), subtract = c("dam" = "sire"))
VDmat <- extract_matrix(model$VCV, format_labels("block:interaction"), scale = 4)
VRmat <- extract_matrix(model$VCV, format_labels("units"))
# VPmat <- VBmat + VAmat + VMmat + VDmat + VRmat
